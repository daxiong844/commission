...
describe('BatchCommissionV2', function () {
  let CommissionStorage, batchCommissionV2, owner, addr1, addr2, addr3, mockToken
  ...
  beforeEach(async function () {
    ...
    // 部署BatchCommissionV2合约
    BatchCommissionV2 = await ethers.getContractFactory('BatchCommissionV2')
    batchCommissionV2 = await BatchCommissionV2.deploy()
    // 初始化交易单合约所需的状态
    await batchCommissionV2.initialize(commissionStorage.address)
  ...
  describe('存入佣金', function () {
    it('佣金是eth,一个人,应该执行成功的情况', async function () {
      ...
      // 使用 await 确保等待交易被确认
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos, { value: ethers.utils.parseEther('1') })).to.emit(batchCommissionV2, 'CommissionDeposited')

      // 检查commissionInfos是否正确存储在合约状态中
      const storedCommissionInfos = await batchCommissionV2.userCommissionInfo(owner.address, key, 0)
      ...
      // 确保余额已相应更改
      const userBalance = await ethers.provider.getBalance(batchCommissionV2.address)
    ...
    it('佣金是eth, 接受佣金的人是多个的情况下, 余额不足应该执行失败', async function () {
      ...
      // 确保addr1余额不足
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos, { value: ethers.utils.parseEther('2') })).to.be.revertedWith('Insufficient Ether sent for commission')
    })
    ...
    it('佣金是eth, 接受佣金的人是多个的情况下也能正常执行', async function () {
      ...
      // 使用 await 确保等待交易被确认
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos, { value: ethers.utils.parseEther('3') })).to.emit(batchCommissionV2, 'CommissionDeposited')
      ...
      const storedCommissionInfos2 = await batchCommissionV2.userCommissionInfo(owner.address, key, 1)
    ...
    it('佣金是某个代币,一个人,应该执行成功的情况', async function () {
      ...
      await mockToken.connect(owner).approve(batchCommissionV2.address, ethers.utils.parseEther('1'))
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos)).to.emit(batchCommissionV2, 'CommissionDeposited')
    ...
    it('佣金是某个代币, 接受佣金的人是多个的情况下, 余额不足应该执行失败', async function () {
      ...
      await mockToken.connect(owner).approve(batchCommissionV2.address, ethers.utils.parseEther('100'))

      // 确保addr1余额不足
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos)).to.be.revertedWith('Insufficient token balance')
    })
    ...
    it('佣金是eth和某个代币, 是否能正常执行', async function () {
      ...
      const initialBalance = await ethers.provider.getBalance(batchCommissionV2.address)
      await batchCommissionV2.connect(owner).depositCommission(key, commissionInfos, { value: ethers.utils.parseEther('1') })
      const finalBalance = await ethers.provider.getBalance(batchCommissionV2.address)
    ...
    it('调用者传入的佣金数组不应为空', async function () {
      ...
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos)).to.be.revertedWith('Commission array should not be empty')
    })
    ...
    it('调用者发送佣金的以太币必须足够', async function () {
      ...
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos)).to.be.revertedWith('Insufficient Ether sent for commission')
    })
    ...
    it('允许调用者转移到该合约的代币数量必须足够', async function () {
      ...
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos)).to.be.revertedWith('The number of tokens that the caller is allowed to transfer to this address is not enough.')
    })
    ...
    it('向addr2转移erc20token的情况下,佣金是eth,是否能正常进行', async function () {
      ...
      // 授权ERC20代币的转让
      await mockToken.connect(owner).approve(batchCommissionV2.address, ethers.utils.parseEther('2'))

      // 发送足够的 ETH 来支付佣金
      const ethAmount = ethers.utils.parseEther('1')
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos, { value: ethAmount })).to.emit(batchCommissionV2, 'CommissionDeposited')
    ...
    it('传入相同的key,则应覆盖之前的commissionInfos,并返回用余额', async function () {
      ...
      // 第一次调用
      await mockToken.connect(owner).approve(batchCommissionV2.address, ethers.utils.parseEther('1'))
      await expect(batchCommissionV2.connect(owner).depositCommission(key, commissionInfos1)).to.emit(batchCommissionV2, 'CommissionDeposited')

      // 获取 owner 地址在 mockToken 合约中的余额
      const balanceBeforeFirstDeposit = await mockToken.balanceOf(owner.address)

      // 确保commissionInfos正确存储在合约状态中
      const storedCommissionInfos1 = await batchCommissionV2.userCommissionInfo(owner.address, key, 0)
      ...
      // 第二次调用，传入相同的key
      await batchCommissionV2.connect(owner).depositCommission(key, commissionInfos2, { value: ethers.utils.parseEther('1') })

      // 确保commissionInfos1正确存储在合约状态中
      const storedCommissionInfos2 = await batchCommissionV2.userCommissionInfo(owner.address, key, 0)
      expect(storedCommissionInfos2[0]).to.equal(addr1.address)
      expect(storedCommissionInfos2[1]).to.equal(ETH)
      expect(storedCommissionInfos2[2]).to.equal(ethers.utils.parseEther('1'))
  ...
  describe('批量下发佣金', function () {
    it('佣金是eth,一个人,应该执行成功的情况', async function () {
      ...
      // 下发佣金
      await expect(batchCommissionV2.connect(owner).distributeCommission(owner.address, key)).to.emit(batchCommissionV2, 'CommissionDistributed')
    ...
    it('佣金数组不能为空', async function () {
      ...
      // 尝试用空数组分配佣金
      await expect(batchCommissionV2.connect(owner).distributeCommission(addr1.address, key)).to.be.revertedWith('Commission array should not be empty')
    })
    ...
    it('当传入一个错误的key值时', async function () {
      ...
      // 下发佣金
      await expect(batchCommissionV2.connect(owner).distributeCommission(owner.address, errorKey)).to.be.revertedWith('Commission array should not be empty')
    })
  })
  ...
  describe('批量摧毁佣金', function () {
    it('佣金是eth,一个人,应该执行成功的情况', async function () {
      ...
      // 摧毁佣金之前相应余额
      const userBalance1 = await ethers.provider.getBalance(batchCommissionV2.address)
      expect(userBalance1).to.equal(ethers.utils.parseEther('1'))

      // 摧毁佣金
      await expect(batchCommissionV2.connect(owner).destroyAndReturnCommission(key)).to.emit(batchCommissionV2, 'CommissionReturned')

      // 摧毁佣金之后相应余额
      const userBalance2 = await ethers.provider.getBalance(batchCommissionV2.address)
    ...
    it('当传入一个错误的key值时', async function () {
      ...
      // 摧毁佣金
      await expect(batchCommissionV2.connect(owner).destroyAndReturnCommission(errorKey)).to.emit(batchCommissionV2, 'CommissionReturned')
    })
  })
})
